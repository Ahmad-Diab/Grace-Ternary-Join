//
// Created by Ahmed Diab on 08.03.25.
//
...
std::unique_ptr<external_join::CounterOperator> constructJoinOperator(Database& db) {
    ...
    external_join::Table &r = db.GetTable("facebook_R");
    external_join::Table &s = db.GetTable("facebook_R");
    external_join::Table &t = db.GetTable("facebook_R");
...
int main() {
    ...
    db.Create(std::string(BENCHMARK_RESOURCES_DIR) +  "facebook/", "dbconfig.csv");
    std::unique_ptr<CounterOperator> topOperator = constructJoinOperator(db);
    assert(topOperator);
    std::function<JoinBenchmarkCounter()> f2 = [&topOperator]() {
        topOperator->Prepare();
        topOperator->Produce();
        return topOperator->getBenchmarkCounter();
    };
    benchmarkFramework.addTask("facebook-binary-grace-external-hash-join", &f2, 88234);
